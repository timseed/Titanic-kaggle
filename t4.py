...
def LoadCsvFile(f):
    ...
    avg_fare = np.zeros(3)
    for pc in range(0, 3):
        avg_fare[pc] = train[(train.Pclass == pc + 1)]['Fare'].dropna().mean()
    #
    # Now set prices
    for pc in range(0, 3):
    ...
    train_exp['Adult'] = train['AgeFill'].map(lambda x: IsAdult(x))
...
tr = LoadCsvFile('train.csv')
ts = LoadCsvFile('test.csv')

#
csf=open("test.csv")
t = csv.reader(csf) 	# Load in the csv file
ids = [] 												# Create a variable to hold the data
for row in t: 							# Skip through each row in the csv file,
    ids.append(row[0]) 								# adding each row to the data variable
ids=ids[1:]
csf.close()
...
print(output)
#predictions_file = open("t.csv", "wb")
...
plt.xlim([-1, fcnt])
print(fittedforest.score(tr[0::, 1::], tr[0::, 0]))
plt.show()
